...
class AdminAnalytics(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        data = {
            ...
            "total_platform_revenue": float(Wallet.objects.all().aggregate(total_amount=Sum('amount'))['total_amount'])- float(Wallet.objects.all().aggregate(total_amount=Sum('balance'))['total_amount']) or 0,
